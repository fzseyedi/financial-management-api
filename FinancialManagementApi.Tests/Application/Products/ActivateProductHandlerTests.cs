...
public sealed class ActivateProductHandlerTests
{
    ...
    private readonly IValidator<ActivateProductCommand> _validator = new ActivateProductCommandValidator();

    [Fact]
    public async Task HandleAsync_Should_Activate_Product_When_Product_Is_Inactive()
    {
        ...
        var handler = new ActivateProductHandler(_productRepositoryMock.Object, _validator);

        await handler.HandleAsync(new ActivateProductCommand(1), CancellationToken.None);
    ...
    public async Task HandleAsync_Should_Throw_NotFoundException_When_Product_Does_Not_Exist()
    {
        ...
        var act = async () => await handler.HandleAsync(new ActivateProductCommand(99), CancellationToken.None);
    ...
    [Fact]
    public async Task HandleAsync_Should_Not_Update_When_Product_Is_Already_Active()
    {
        var product = new Product("PRD-001", "Office Chair", 150.00m);
        typeof(FinancialManagementApi.Domain.Common.BaseEntity)
            .GetProperty(nameof(FinancialManagementApi.Domain.Common.BaseEntity.Id))!
